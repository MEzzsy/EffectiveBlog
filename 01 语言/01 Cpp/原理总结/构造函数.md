...
>   TODO 经测试并没有调用复制构造函数，书和C++ primer都有介绍说会调用，这部分还需要确认。
...
```cpp
void constructor::test2() {
    X x = returnX();
}

constructor::X::X(const constructor::X &x) {
    my_log("X(const X &x)");
}
...
constructor::X constructor::X::operator=(const constructor::X &x) const {
    my_log("X operator=(const X &x)const");
    return x;
}

constructor::X constructor::returnX() {
    X xx;
    return xx;
}
