...
## put：定位桶后选择 CAS 或桶级加锁

1. **校验参数并计算 hash。** 键或值为 `null` 时抛出异常，通过 `spread(key.hashCode())` 得到扰动后的 hash。
2. **数组未初始化时，调用 `initTable()`。** 通过 CAS 将 `sizeCtl` 改为 -1 竞争初始化资格，成功后分配数组并设置扩容阈值，然后回到插入循环。
3. **定位桶，桶为空时用 CAS 插入。** 数组可用后，以 `(table.length - 1) & hash` 计算下标，并通过 `tabAt` 以 volatile 语义读取桶头；确认为空后，`casTabAt` 尝试把该位置从 `null` 改为新节点。成功就结束桶内插入，失败则重新读取状态。
4. **桶头为 `MOVED` 时，转向扩容路径。** 这说明当前旧桶已被处理，调用 `helpTransfer` 尝试协助迁移，并**取得新表继续插入**。是否实际参与迁移，取决于是否还有待领取任务等条件。
5. **普通非空桶通过 `synchronized (f)` 加锁。** 获得锁后必须验证 `tabAt(tab, i) == f`，因为等待期间桶头可能已被删除、树化或替换。验证失败就重试；验证成功后，链表路径遍历查找，已有键更新 `val`，没有则尾插；树桶调用 `TreeBin.putTreeVal`。
6. **完成树化检查与计数更新。** 达到 `binCount >= 8` 时调用 `treeifyBin`，但数组长度不足 64 时先尝试扩容。在这里的链表新增路径中，`binCount` 是插入前的长度，因此向已有 8 个节点的链表新增第 9 个节点时会请求树化。覆盖旧值时返回旧值，只有新增映射才调用 `addCount(1L, binCount)`，并按该方法的执行路径决定是否检查扩容。

分段锁：从并发控制看，空桶依靠 CAS 竞争数组位置，非空桶依靠桶头锁协调更新。对已有键的更新同样会进入非空桶加锁路径，不能把锁理解为只有插入冲突节点时才使用。

## get：读取桶头，再按节点类型查找

`get(key)` 先计算扰动后的 hash，通过 `tabAt` 读取目标桶头。它不会为了读取而初始化数组，也不获取桶的 `synchronized` 锁。

1. **数组未初始化或桶为空，返回 `null`。** 如果传入的 key 本身为 `null`，则在调用 `key.hashCode()` 时抛出异常。
2. **先检查桶头。** 桶头的 hash 与 key 均匹配时，直接返回其 volatile 字段 `val`。
3. **普通链表沿 `next` 查找。** 桶头没有命中且是普通节点时，逐个比较 hash 和 key，找到后返回值，遍历结束仍未找到则返回 `null`。
4. **负 hash 的特殊节点交给 `find`。** `ForwardingNode.find` 沿新表引用继续查找，必要时跨越多轮扩容；`TreeBin.find` 根据内部锁状态选择查树或沿链表查找；`ReservationNode.find` 返回 `null`，表示计算中的映射尚未发布。

`TreeBin.find` 查树前通过 CAS 增加读计数，结束后释放；如果观察到树写锁被持有或有写线程等待，就沿链表查找，不阻塞等待这把写锁。因此，`get` 不获取桶锁，但树路径仍有读写协调。

读取的安全性来自数组元素的 volatile 访问、节点的 final/volatile 字段以及节点发布顺序。扩容时，已经取得旧链表的线程可以继续遍历；读到转发节点的线程则去新表查找。**`get` 只查数据，不帮助扩容，也不需要等待整张表迁移完成。**

## remove：加锁查找，删除成功后减少计数

1. **计算 hash 并读取桶头。** 数组或目标桶不存在时返回 `null`，不会为了删除而创建数组。
2. **遇到转发节点时切换数组。** 与 `put` 一样，通过 `helpTransfer` 尝试帮助迁移，然后在新表继续查找。
3. **尝试获取桶头锁。** 桶非空且桶头不是转发节点时，执行 `synchronized (f)`，获取此前读到的桶头对象的监视器锁。普通链表的锁对象是头节点，树桶的锁对象是 `TreeBin`。如果锁被其他线程持有，当前线程必须等待获得锁后，才能进入同步块；即使待删除的 key 不存在，也需要先加锁再查找。
4. **获得锁后重新验证桶头。** 检查 `tabAt(tab, i) == f`，确认锁住的对象仍是当前桶头。若不相等，`validated` 保持 false，退出同步块、释放锁，再回到外层循环读取新的桶状态；只有验证通过，才进入链表或树的查找删除逻辑。
5. **根据结构删除节点。** 链表中删除非头节点时，令 `pred.next = e.next`；删除头节点时，通过 `setTabAt` 发布新的桶头。树桶先查找目标 `TreeNode`，再调用 `removeTreeNode` 删除，必要时将剩余节点转回链表。
6. **释放桶头锁，删除成功后更新计数。** 退出 `synchronized` 块时自动释放锁；实际删除成功后，在同步块外调用 `addCount(-1L, -1)` 并返回旧值。已经检查当前桶但未找到目标键时返回 `null`。`check = -1` 跳过扩容检查，删除也不会触发自动缩容。

删除通过桶锁协调写操作，读线程仍可并发查找。与删除重叠的读取可能已经取得被删节点并返回旧值，这不等于删除失效；线程安全并不要求所有并发读取都等待删除结束。

## 扩容：多线程领取区间，迁移后通过转发节点衔接新旧表

扩容可以由两类路径推动：

- 新增映射后的 `addCount` 在执行到扩容检查、发现数量达到阈值时发起扩容；
- `tryPresize` 则用于预分配或容量调整，例如树化时发现数组长度不足 64。

**每一轮实际迁移由 `transfer` 完成，新数组长度是旧数组的两倍。**

### 发起扩容并记录参与状态

设 `R = resizeStamp(n) << RESIZE_STAMP_SHIFT`，发起者通过 CAS 将 `sizeCtl` 从扩容阈值改为 `R + 2`，然后调用 `transfer(tab, null)`。`transfer` 创建新数组，设置 `nextTable` 和 `transferIndex = n`。

其他写线程可以通过 `helpTransfer` 或 `addCount` 加入：确认状态允许且仍有工作可领取后，先用 CAS 将 `sizeCtl` 加一，再调用 `transfer(tab, nextTable)`。高位的扩容戳与低位的参与计数共同组成扩容状态，不能把整个负数直接解释为线程数量。

### 通过 transferIndex 分配工作

每个线程根据 `stride` 领取一段桶下标，通过 CAS 将 `transferIndex` 向前移动，获得区间 `[nextBound, nextIndex)`，随后从该区间的尾部向前迁移。

一个区间完成后可以继续领取下一段。`transferIndex <= 0` 只说明区间已被分配完，不代表各线程已完成手中的任务；因此还需要参与线程计数来判断何时可以提交新表。

### 迁移单个桶并发布转发节点

1. **空桶直接 CAS 放入 `ForwardingNode`。** 这样后续插入会转向新表；如果 CAS 失败，则重新检查该桶。

2. **已经是 `ForwardingNode` 就跳过。** 该桶已完成处理， 继续处理下一个位置。

3. **非空且不是 `ForwardingNode` 的桶，先获取旧桶头锁。** 执行 `synchronized (f)`，锁被占用时等待；获得锁后检查 `tabAt(tab, i) == f`。如果不相等，说明等待期间桶头发生了变化，退出同步块、释放锁，再重新检查当前下标 `i`，不能继续使用已经失效的桶头进行迁移。

4. **桶迁移**

   - 链表节点

     1. 类似HashMap，将链表分为新的两段，放到新的数组中。

     2. 迁移不会改写旧普通节点的 `next`，已经持有旧链表引用的读线程仍可继续遍历。

   - 树节点

     - 类似HashMap，把迁移结果写入新数组。不超过 `UNTREEIFY_THRESHOLD`（6），会转为链表。

5. **在原数组 `tab[i]` 放置 `ForwardingNode`，然后才释放锁。** 当前线程仍处于 `synchronized (f)` 内，执行 `setTabAt(tab, i, fwd)`，把原数组该位置的桶头替换为持有 `nextTab` 引用的 `ForwardingNode`。这里替换的是 `tab[i]` 的引用，锁住的仍是原来的 `f` 对象。读线程通过数组元素访问的 volatile 语义和上述发布顺序，在读到 `ForwardingNode` 后安全转向已经发布的新桶。

因此，不只是原本为空的桶需要放置 `ForwardingNode`：空桶通过 CAS 放置，原本非空的链表桶和树桶则在迁移成功后、释放桶头锁之前，通过 `setTabAt(tab, i, fwd)` 放置。它统一表示原数组的该位置已经处理，后续访问应转向新数组。

### 最后一个线程扫描并提交

线程完成手中的任务且无法再领取区间时，通过 CAS 将 `sizeCtl` 减一。若减一前的状态满足 `sc - 2 == R`，说明当前线程是最后退出常规迁移阶段的线程。

它设置 `finishing = true`，重新扫描旧数组，确认所有桶都处理完毕（每个位置都变成 `ForwardingNode`），再清空 `nextTable`、令 `table = nextTab`，并恢复新容量对应的扩容阈值。至此，本轮扩容完成。

扩容与其他操作衔接的关键是桶头状态：`put`、`remove` 遇到转发节点后可以帮助迁移并在新表重试；`get` 沿转发节点继续查找。各线程不需要先等待整个扩容结束，才能访问已经准备好的新桶。

# 问题

每个操作里都有循环获取桶头，那么是不是存在长时间获取不到正确桶头的情况
