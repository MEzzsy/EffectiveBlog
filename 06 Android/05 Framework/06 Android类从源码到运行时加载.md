...
| `PathClassLoader` | Android 默认用来加载应用 APK、split 和系统库代码 | 应用默认加载器通常在进程启动、绑定应用期间按需创建，早于 `Application` 实例化；系统库和隔离 split 的加载器有各自的创建时机 |
...
通过 ART 的引导加载机制，加载 boot class path 中的 Java 核心库和 Android Framework 基础类，为应用提供系统级基础类型。[BootClassLoader 源码](https://android.googlesource.com/platform/libcore/+/refs/heads/master/ojluni/src/main/java/java/lang/ClassLoader.java)
...
Java 层的 `BootClassLoader` 采用懒加载单例：首次调用 `BootClassLoader.getInstance()` 时执行 `new BootClassLoader()`，后续调用返回已有实例。这个实例通常在 Zygote 初始化期间就已准备好，子进程继承相应的内存状态，因此不需要为每个应用重新创建一个 `BootClassLoader`。例如，Zygote 预先缓存部分系统库加载器的流程，就会通过 `ApplicationLoaders` 获取系统 ClassLoader 及其 parent。[ZygoteInit 源码](https://android.googlesource.com/platform/frameworks/base/+/master/core/java/com/android/internal/os/ZygoteInit.java)

> 这里说的是 Java 层加载器对象的创建；ART 对核心类的引导加载能力属于运行时机制，不能理解成必须先创建这个 Java 对象，才能加载 `java.lang.ClassLoader` 等核心类。
>

### PathClassLoader：应用绑定期间按需创建并复用

#### 主要用途

作为应用的默认加载器，从应用 APK 及已纳入类路径的 split 中加载业务类和随应用打包的依赖库类；系统也可以用其他 `PathClassLoader` 实例加载非 boot class path 的库。[PathClassLoader 源码](https://android.googlesource.com/platform/libcore/+/master/dalvik/src/main/java/dalvik/system/PathClassLoader.java)

在普通应用的默认加载模式下，例如：

- 应用自己的类：`com.example.MainActivity`、`com.example.UserRepository`。
- 随应用打包的依赖类：`androidx.appcompat.app.AppCompatActivity`、`com.google.gson.Gson`。
- 已安装且代码路径已加入加载器的功能模块类：例如订单模块的 `com.example.order.OrderActivity`。

例如，`MainActivity` 继承 `AppCompatActivity`，后者再间接继承 `android.app.Activity`：前两者的代码随应用打包，通常由应用 `PathClassLoader` 加载；`android.app.Activity` 属于系统 Framework，由引导加载体系负责。**判断依据是类的代码来源及加载路径，而不是它是否用于 Android 开发。**

#### 加载时机

对于普通应用的默认加载器，典型时机是 Zygote fork 出应用进程后，`ActivityThread.handleBindApplication()` 处理应用绑定、准备应用 Context 和代码环境的阶段。加载器必须在实例化应用的 `Application` 类之前准备好，因此也早于 `Application.attachBaseContext()` 和 `Application.onCreate()`。[ActivityThread 源码](https://android.googlesource.com/platform/frameworks/base/+/master/core/java/android/app/ActivityThread.java)



从源码看，`LoadedApk.getClassLoader()` 会先检查缓存；尚未创建时，才进入 `createOrUpdateClassLoaderLocked()`。后续创建链路可以简化为：
...
创建后，`LoadedApk` 保存加载器供后续使用。同一应用进程中的普通 Activity、Service 等组件通常复用它，并不会在每次组件启动时重新创建。[LoadedApk 源码](https://android.googlesource.com/platform/frameworks/base/+/refs/heads/main/core/java/android/app/LoadedApk.java)、[ApplicationLoaders 源码](https://android.googlesource.com/platform/frameworks/base/+/refs/heads/main/core/java/android/app/ApplicationLoaders.java)、[ClassLoaderFactory 源码](https://android.googlesource.com/platform/frameworks/base/+/master/core/java/com/android/internal/os/ClassLoaderFactory.java)

`PathClassLoader` 是一种加载器类型，不对应唯一的创建阶段：

- 系统 ClassLoader 也使用 `PathClassLoader`：首次调用 `ClassLoader.getSystemClassLoader()` 时，通过内部静态类初始化触发 `createSystemClassLoader()`，根据 `java.class.path` 创建；它与应用 APK 的默认加载器是不同实例。
- 非 boot class path 的共享系统库可以有独立加载器；部分由 Zygote 提前创建并缓存，其他按依赖需求创建或复用。
- 启用 isolated split 后，feature split 的加载器通常在首次请求该 split 的 Context、资源或代码环境时，连同所需依赖按需创建并缓存；默认 split 模式通常复用应用加载器。

上述差异分别可见 [ClassLoader 源码](https://android.googlesource.com/platform/libcore/+/refs/heads/master/ojluni/src/main/java/java/lang/ClassLoader.java)、[ApplicationLoaders 的系统库缓存逻辑](https://android.googlesource.com/platform/frameworks/base/+/refs/heads/main/core/java/android/app/ApplicationLoaders.java)和 [LoadedApk 的 SplitDependencyLoaderImpl](https://android.googlesource.com/platform/frameworks/base/+/refs/heads/main/core/java/android/app/LoadedApk.java)。

### DexClassLoader：业务需要动态代码时显式创建

`DexClassLoader` 没有统一的系统启动创建时机。应用或插件框架准备好 DEX、APK、JAR 路径后，通过 `new DexClassLoader(...)` 创建实例，parent 通常传入宿主应用的 ClassLoader。例如，插件可以在应用启动阶段初始化，也可以在用户首次进入相关功能时才创建加载器。[DexClassLoader 源码](https://android.googlesource.com/platform/libcore/+/master/dalvik/src/main/java/dalvik/system/DexClassLoader.java)
