...
- `handleBindApplication()` 先准备应用的 Context 和资源访问环境，期间按需创建应用的 ClassLoader。
- 随后通过 `LoadedApk.makeApplicationInner()` 获取应用的 ClassLoader，用它加载并实例化 Application，再调用 `attachBaseContext()` 绑定 Context。
- 安装本进程在此次绑定中需要初始化的 ContentProvider，并执行这些 Provider 的 `onCreate()`。
- 最后调用 `Application.onCreate()`。

## 初始化时的 ClassLoader 创建

**应用的 ClassLoader 由 LoadedApk 按需创建并缓存，在 Application 实例化之前就已准备好。**`LoadedApk.getClassLoader()` 在 `mClassLoader` 为空时调用 `createOrUpdateClassLoaderLocked()`，后续获取时复用已有对象。

普通应用默认使用 **PathClassLoader**，父加载器通常是 **BootClassLoader**。

下面是 AOSP 普通应用路径的简化示意，省略锁、路径整理和特殊分支；具体方法签名随 Android 版本变化：

```java
// LoadedApk：首次获取时创建，之后复用
...
private void createOrUpdateClassLoaderLocked(...) {
    ... // 根据 ApplicationInfo 整理代码路径、native 库路径及共享库依赖
...
private void handleBindApplication(AppBindData data) {
    ...
    // 准备资源环境时，也可能通过 LoadedApk.getClassLoader() 触发类加载器创建
...
public Resources getResources() {
    if (mResources == null) {
        ...
        mResources = ResourcesManager.getInstance().getResources(
                ...
                getClassLoader(), null); // 获取或按需创建应用 ClassLoader
    }
