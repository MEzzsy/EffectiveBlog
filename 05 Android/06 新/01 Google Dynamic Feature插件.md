...
此时代码集合在当前进程的生命周期内基本不变。某个类是否位于 DEX、某个资源是否位于资源表，通常在构建和启动阶段就已经确定。完整的普通类加载链路见 [Android 类从源码到运行时加载](<02 Android类从源码到运行时加载.md>)。
...
Kotlin/Java 源码如何变成 DEX、系统如何在进程启动时创建应用 ClassLoader，以及 ART 如何完成类的加载、链接和初始化，见 [Android 类从源码到运行时加载](<02 Android类从源码到运行时加载.md>)。这里仅说明 Dynamic Feature 相比普通启动流程多出的步骤。
