...
4. **singleInstance**，单实例模式。一种加强的singleTask模式，除了具有singleTask的特性，还加强了一点，就是此模式下的Activity只能单独位于一个栈中。**适用新开Activity和app能独立开的，如系统闹钟**
...
全局的gradle配置文件，这里的属性会影响到项目中所有的gradle编译脚本。
...
每个线程都有一个ThreadLocalMap，ThreadLocalMap里有个数组table存放ThreadLocal性质（Entry）的对象。

Entry内部只有value，因为key就是ThreadLocal。

ThreadLocal的set方法放入需要的值，先获取Thread，再取出这个Thread的ThreadLocalMap。既然叫Map，那操作和Map差不多。先根据hash值和table数组长度获取数组位置，如果产生hash冲突，这里的解决办法是位置+1，直到找到一个null的位置。如果超出了数组长度就放在0位置，然后扩容。扩容是容量*2，具体细节没必要深入。
...
[跨进程通信笔记](https://mezzsy.github.io/2019/08/06/Android/跨进程通信笔记/)
...
有不懂的地方可以看笔记：[跨进程通信笔记](https://mezzsy.github.io/2019/08/06/Android/跨进程通信笔记/)
